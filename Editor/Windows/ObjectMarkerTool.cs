...
namespace ArgonautJH.ObjectMarkerTool.Editor
{
    ...
    {
        // 상수 정의
        private const string DEFAULT_EXPORT_PATH = "Assets/TempScript";
        private const string DEFAULT_CUSTOM_MATERIAL_FOLDER = "Assets/CustomMaterials";
        private const string EXPORTED_COMPONENT_NAME = "ExportedObjectComponent";
        private const string EXPORTED_COMPONENT_FULL_TYPE = "ArgonautJH.ObjectMarkerTool.Runtime.ExportedObjectComponent";
        ...
        // 리플렉션 타입 캐싱
        private static Type _cachedExportedType = null;
        ...
        private void DrawExportPathSection()
        {
            ...
            if (GUILayout.Button("스크립트 추출 경로 선택"))
            {
                ...
                if (string.IsNullOrEmpty(_exportScriptFolder))
                {
                    _exportScriptFolder = DEFAULT_EXPORT_PATH;
                }
            }
            EditorGUILayout.LabelField("스크립트 추출 경로:", _exportScriptFolder);
            ...
            if (GUILayout.Button("스크립트 추출하기"))
            {
                if (_exportScriptFolder.Equals(DEFAULT_EXPORT_PATH))
                {
                    if (!AssetDatabase.IsValidFolder(DEFAULT_EXPORT_PATH))
                    {
                        AssetDatabase.CreateFolder("Assets", "TempScript");
                    }
                }
        ...
        private void CreateCustomObject()
        {
            ...
            GameObject newObj = CreatePrimitiveObject(spawnPosition, "CustomObject");
            ApplyMaterialToObject(newObj, selectedPreset);
            CreateTextChild(newObj);

            if (useExportedComponent)
            {
                AddExportedComponent(newObj, _summary, _text);
            }
            else
            {
                // 복사 실패 시 기본 ObjectComponent 사용
        ...
        private Vector3 GetSpawnPosition()
        {
            ...
            if (SceneView.lastActiveSceneView != null)
            {
                ...
                float distance = 3f;
                spawnPos = sceneCam.transform.position + sceneCam.transform.forward * distance;
            }
            return spawnPos;
        }
        ...
        /// 기본 큐브 오브젝트 생성
        /// </summary>
        private GameObject CreatePrimitiveObject(Vector3 position, string objectName)
        {
        ...
        /// <summary>
        /// 프리셋에 따른 Material 생성 및 적용
        /// </summary>
        private void ApplyMaterialToObject(GameObject obj, MaterialPreset preset)
        {
            Renderer renderer = obj.GetComponent<Renderer>();
            if (renderer != null)
            {
                renderer.material = GetOrCreateMaterial(preset);
            }
        }

        /// <summary>
        /// 큐브의 자식으로 3D 텍스트 오브젝트 생성
        /// </summary>
        private void CreateTextChild(GameObject parent)
        {
        ...
        /// <summary>
        /// ExportedObjectComponent를 추가하고 데이터 설정
        /// </summary>
        private void AddExportedComponent(GameObject obj, string summary, string text)
        {
            Type exportedType = GetExportedObjectComponentType();
            if (exportedType != null)
            {
                var comp = obj.AddComponent(exportedType);
                var summaryField = exportedType.GetField("summary");
                var textField = exportedType.GetField("text");
                if (summaryField != null)
                    summaryField.SetValue(comp, summary);
                if (textField != null)
            ...
            if (!File.Exists(targetFile))
            {
                ...
                if (File.Exists(sourcePath))
                {
                    try
                    {
                        ...
                        string newCode = sourceCode.Replace("public class ObjectComponent", "public class " + EXPORTED_COMPONENT_NAME);
                        // 변경된 코드를 대상 파일에 저장
        ...
        {
            if (_cachedExportedType != null) return _cachedExportedType;

            foreach (var assembly in AppDomain.CurrentDomain.GetAssemblies())
            {
                Type t = assembly.GetType(EXPORTED_COMPONENT_FULL_TYPE);
                if (t != null)
                {
                    _cachedExportedType = t;
                    return t;
                }
            }
            return null;
        ...
        private Material GetOrCreateMaterial(MaterialPreset preset)
        {
            ...
            else
            {
                ...
                if (!AssetDatabase.IsValidFolder(DEFAULT_CUSTOM_MATERIAL_FOLDER))
                {
                    AssetDatabase.CreateFolder("Assets", "CustomMaterials");
                }
                string assetPath = $"{DEFAULT_CUSTOM_MATERIAL_FOLDER}/{preset.PresetName}.mat";
                AssetDatabase.CreateAsset(newMat, assetPath);
                AssetDatabase.SaveAssets();
            }
